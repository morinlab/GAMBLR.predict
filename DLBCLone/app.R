...
#this part is just temporary for some testing
lyseq_status = read_tsv(file=paste0(here::here(),"/lyseq_status_validation.tsv")) %>% column_to_rownames("sample_id")
lyseq_meta = read_tsv(file=paste0(here::here(),"/lst.lymphgen_calls.no_cnvs.with_sv.no_A53.tsv")) %>%
...
prototypes = lyseq_status %>% select(any_of(colnames(full_status)))
missing = colnames(full_status)[!colnames(full_status) %in% colnames(prototypes)]
for(colmis in missing){
  prototypes[[colmis]] = 0
}

bcl2_cases = filter(lyseq_meta,BCL2.Translocation=="yes",sample_id %in% rownames(prototypes)) %>% pull(sample_id)

bcl6_cases = filter(lyseq_meta,BCL6.Translocation=="yes",sample_id %in% rownames(prototypes)) %>% pull(sample_id)
prototypes[bcl2_cases,"BCL2_SV"] = maxval
prototypes[bcl6_cases,"BCL6_SV"] = maxval
...
build_query <- function(params) {
  ...
  observeEvent(input$predict, {
    ...
    if (identical(input$predict_source, "Demo sample")) {
      ...
      if(input$predict_sample %in% lyseq_meta$sample_id){
        filter(lyseq_meta,sample_id==input$predict_sample) %>%
          select(Subtype.Prediction, ends_with("Features")) %>%
          print()
      }
      ...
      sample_class <- predicted$unlabeled_predictions %>%
        ...
        select(sample_id, DLBCLone_ko, EZB:Other_score, valid_classes, score_ratio)
