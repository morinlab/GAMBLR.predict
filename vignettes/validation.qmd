---
title: "DLBCLone Validation Data Generation and Example"
vignette: >
...
lyseq_features_optimized_train <- DLBCLone_activate( # Activates optimized model setting the UMAP projection
  lyseq_features_optimized_train, 
  force = TRUE
)
```
...
pred_valid_lyseq <- DLBCLone_predict(
  mutation_status = lyseq_valid[test_sample,],
...
Finally, lets assess the accuracy of the validation set by comparing `dlbclone_predict` predictions with its ground truth lymphgen labeling found in its metadata. To do this we should first get predictions from `dlbclone_predict` on all of the 148 lyseq panel validation samples.
...
pred_vs_original = left_join(pred_valid_lyseq_all$prediction,valid_metadata_vaf)
