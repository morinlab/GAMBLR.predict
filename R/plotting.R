...
#' @param colour_by Column name in \code{dlbclone_umap_out$df} to color points by (default: "lymphgen"). 
#' Set to NULL for no colouring.
#' @param default_colour Colour used for feature-negative points when
#'   \code{colour_by = NULL}. Defaults to \code{"grey70"}.
...
umap_feature_plot <- function(dlbclone_umap_out,
  ...
  meta_cols <- c("sample_id", "V1", "V2")
  if (!is.null(colour_by)) {
    meta_cols <- c(meta_cols, colour_by)
  }

  meta_no_features <- dlbclone_umap_out$df %>%
    select(all_of(unique(meta_cols)))
  ...
  for (i in seq_along(features)) {
    ...
    df$.feature_state <- ifelse(df[[feature]] > 0, feature, "no")

    feature_col <- if (feature %in% names(lgcol)) lgcol[[feature]] else "orange"

    p <- ggplot(df, aes(V1, V2))
    if (is.null(colour_by)) {
      p <- p + geom_point(
        data = df[df$.feature_state == "no", , drop = FALSE],
    ...
    p <- p +
      geom_point(
        data = df[df$.feature_state == feature, , drop = FALSE],
        colour = feature_col,
        size = 1.3,
        show.legend = FALSE
      ) +
    ...
    if (!is.null(colour_by)) {
      p <- p + scale_colour_manual(values = lgcol)
    }
